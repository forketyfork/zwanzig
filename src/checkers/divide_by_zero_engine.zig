...
pub const DivideByZeroEngineChecker = struct {
    ...
    fn analyzeFunction(
        ...
        defer engine.deinit();
        engine.setCheckerName("divide-by-zero-engine");
...
test "DivideByZeroEngineChecker initialization" {
    const testing = std.testing;
    try testing.expectEqualStrings("divide-by-zero-engine", DivideByZeroEngineChecker.checker.name);
    try testing.expectEqual(checker_mod.Severity.err, DivideByZeroEngineChecker.checker.default_severity);
}
