...
pub const FunctionSummary = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, fn_ast_node: u32) FunctionSummary {
        return .{
...
pub const SummaryCache = struct {
    ...
    summaries: std.AutoHashMap(u32, *FunctionSummary),
    ...
    pub fn init(allocator: std.mem.Allocator) SummaryCache {
        return .{
            .summaries = std.AutoHashMap(u32, *FunctionSummary).init(allocator),
            .allocator = allocator,
            .cache_hits = 0,
            .cache_misses = 0,
        };
    ...
    pub fn get(self: *SummaryCache, fn_ast_node: u32) ?*FunctionSummary {
    ...
    pub fn has(self: *const SummaryCache, fn_ast_node: u32) bool {
...
pub const AnalysisEngine = struct {
    ...
    pub fn getOrComputeSummary(self: *AnalysisEngine, fn_ast_node: u32) ?*FunctionSummary {
        // Check cache first
    ...
    fn computeSummary(self: *AnalysisEngine, fn_ast_node: u32) EngineError!?FunctionSummary {
...
// ============== Function Summary and Cache Tests ==============

test "FunctionSummary basic operations" {
    const allocator = std.testing.allocator;

    var summary = FunctionSummary.init(allocator, 42);
    defer summary.deinit();

    try std.testing.expectEqual(@as(u32, 42), summary.fn_ast_node);
...
test "FunctionSummary preconditions and postconditions" {
    ...
    var summary = FunctionSummary.init(allocator, 1);
    defer summary.deinit();

    // Add precondition
    try summary.addPrecondition(Constraint.intCompare(10, .ge, 0));
    try std.testing.expectEqual(@as(usize, 1), summary.preconditions.items.len);

    // Add postcondition
    try summary.addPostcondition(Constraint.intCompare(20, .eq, 100));
...
test "FunctionSummary applicability check" {
    ...
    try state.setVar(10, .{ .concrete_int = 5 });
    try std.testing.expect(summary.isApplicable(&state));
...
test "SummaryCache basic operations" {
    ...
    // Initially empty
    try std.testing.expect(!cache.has(42));
...
test "SummaryCache clear" {
    ...
    // Add some summaries
    const summary1 = FunctionSummary.init(allocator, 1);
    const summary2 = FunctionSummary.init(allocator, 2);
    ...
    try std.testing.expect(!cache.has(1));
    try std.testing.expect(!cache.has(2));
}
...
test "SummaryCache reuse across call sites" {
    ...
    // Add a summary
    var summary = FunctionSummary.init(allocator, 100);
    summary.markPure();
    try cache.put(summary);
...
test "FunctionSummary apply to state" {
    ...
    try summary.addPostcondition(Constraint.intCompare(50, .gt, 0));
